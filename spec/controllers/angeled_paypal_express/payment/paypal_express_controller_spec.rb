...
describe AngelEdPaypalExpress::Payment::PaypalExpressController do
  before do
  ...
  describe "POST ipn" do
    ...
    before do
      backer
      post :ipn, ipn_data.merge({ use_route: 'angel_ed_paypal_express' })
      backer.reload
    end
  ...
  describe "POST notification" do
    context 'when receive a notification' do
      it 'and not found the backer, should return 404' do
        post :notifications, { id: 1, use_route: 'angel_ed_paypal_express'}
        response.status.should eq(404)
      end

      it 'and the transaction ID not match, should return 404' do
        backer = Factory(:backer, payment_id: '1234')
        post :notifications, { id: backer.id, txn_id: 123, use_route: 'angel_ed_paypal_express' }
        response.status.should eq(404)
      end
      ...
      it 'should create a payment_notification' do
        ...
        post :notifications, { id: backer.id, txn_id: 1234 , use_route: 'angel_ed_paypal_express' }
        backer.reload
    ...
    context 'setup purchase' do
      context 'when have some failures' do
        it 'user not logged in, should redirect' do
          ...
          #get :pay, {locale: 'en', use_route: 'angel_ed_paypal_express' }
        ...
        it 'backer not belongs to current_user should 404' do
          ...
          lambda { 
            get :pay, { id: backer.id, locale: 'en', use_route: 'angel_ed_paypal_express' }
        ...
        it 'raise a exepction because invalid data and should be redirect and set the flash message' do
          ...
          get :pay, { id: backer.id, locale: 'en', use_route: 'angel_ed_paypal_express' }
          flash[:failure].should == I18n.t('paypal_error', scope: AngelEdPaypalExpress::Payment::PaypalExpressController::SCOPE)
  ...
  describe "GET cancel" do
    context 'when cancel the paypal purchase' do
      it 'should show for user the flash message' do
        ...
        get :cancel, { id: backer.id, locale: 'en', use_route: 'angel_ed_paypal_express' }
        flash[:failure].should == I18n.t('paypal_cancel', scope: AngelEdPaypalExpress::Payment::PaypalExpressController::SCOPE)
        response.should be_redirect
      end
    end
  end
    ...
    context 'paypal returning to success route' do
      ...
      context 'when paypal purchase is ok' do
        ...
        it 'should update the backer and redirect to thank_you' do
          ...
          get :success, { id: backer.id, PayerID: '123', locale: 'en', use_route: 'angel_ed_paypal_express' }
          backer.reload
